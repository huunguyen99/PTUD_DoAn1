...
{
    public partial class ThongKeThuePhongTheoNam : Form
    {
        public ThongKeThuePhongTheoNam()
        {
            InitializeComponent();
        }

        private void ThongKeThuePhongTheoNam_Load(object sender, EventArgs e)
        {
        ...
        private void fillChart(int year, int type)
        {
            ...
            if (type == 1)
            {
                adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam " + year + "," + 1, con);
            }
            if (type == 2)
            {
                adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam " + year + "," + 2, con);
            }
            if (type == 3)
            {
                adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam " + year + "," + 3, con);
            }
            
            adapt.Fill(ds);
            chartThongKeDoanhThuThang.DataSource = ds;
            //set the member of the chart data source used to data bind to the X-values of the series  
            chartThongKeDoanhThuThang.Series["Số lượng thuê"].XValueMember = "Thang";
            ...
            if (type == 1)
            {
                chartThongKeDoanhThuThang.Titles.Add("SL sinh viên thuê và chưa trả phòng " + "năm " + year);
                chartThongKeDoanhThuThang.Series["Số lượng thuê"].Color = Color.LightGreen;
            }
            if (type == 2)
            {
                chartThongKeDoanhThuThang.Titles.Add("SL sinh viên thuê và đã trả phòng " + "năm " + year);
                chartThongKeDoanhThuThang.Series["Số lượng thuê"].Color = Color.LightGray;
            }
            if (type == 3)
            {
                chartThongKeDoanhThuThang.Titles.Add("SL sinh viên thuê phòng " + "năm " + year);
            ...
            if (hitResult != null)
            {
                ...
                if (hitResult.Series != null && point.YValues[0] != 0)
                {
                    if (hitResult.Series.Points != null) // null pointer exception here
                    {
                        ...
                        if (type == 1)
                        {
                            adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam_ChiTiet " + xValue + "," + year + "," + 1, con);
                        }
                        if (type == 2)
                        {
                            adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam_ChiTiet " + xValue + "," + year + "," + 2, con);
                        }
                        if (type == 3)
                        {
                            adapt = new SqlDataAdapter("Exec ThongKeThuePhongTheoNam_ChiTiet " + xValue + "," + year  + "," + 3, con);
                        }
                        adapt.Fill(ds);
                        var label = "Chi tiết thuê phòng " + xValue + "/" + year;
