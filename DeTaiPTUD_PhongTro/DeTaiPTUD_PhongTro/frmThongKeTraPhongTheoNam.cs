...
{
    public partial class frmThongKeTraPhongTheoNam : Form
    {
        public frmThongKeTraPhongTheoNam()
        {
            InitializeComponent();
        }

        private void frmThongKeTraPhongTheoNam_Load(object sender, EventArgs e)
        {
            DateTime DateNow = DateTime.Now;
            txtYear.Text = DateNow.Year.ToString();
            //cboMonth.ValueMember = 1;
            fillChart(DateNow.Year);
        }
        ...
        private void fillChart(int year)
        {
            ...
            adapt = new SqlDataAdapter("Exec ThongKeTraPhongTheoNam " + year, con);


            adapt.Fill(ds);
            chartThongKeDoanhThuThang.DataSource = ds;
            //set the member of the chart data source used to data bind to the X-values of the series  
            chartThongKeDoanhThuThang.Series["Số lượng trả"].XValueMember = "Thang";
            ...
            chartThongKeDoanhThuThang.Titles.Add("SL sinh viên trả phòng " + "năm " + year);
        ...
        {
            string typeName = "SLTraPhong";
            string typeNameVi = "SL trả phòng";
            ...
            if (hitResult != null)
            {
                ...
                if (hitResult.Series != null && point.YValues[0] != 0)
                {
                    if (hitResult.Series.Points != null) // null pointer exception here
                    {
                        ...
                        adapt = new SqlDataAdapter("Exec ThongKeTraPhongTheoNam_ChiTiet " + xValue + "," + year, con);

                        adapt.Fill(ds);
                        var label = "Chi tiết trả phòng " + xValue + "/" + year;
